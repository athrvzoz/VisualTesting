...
class SimpleVisualTester {
    constructor(baseDir) {
        this.baseDir = baseDir || path.join(__dirname, '../public');
    ...
    async processImage(isFirstRun, baselineDir, currentRunDir, filename, currentPath, label) {
        ...
        try {
            ...
            result.diff = path.join('runs', uniqueRunId, diffFilename);
        } catch (e) {
            result.status = 'error';
        ...
        for (const file of files) {
            if (file.endsWith('.json')) {
                try {
                    ...
                    console.error('Error reading report file:', file, e); // Added error logging for clarity
                }
            }
        }
