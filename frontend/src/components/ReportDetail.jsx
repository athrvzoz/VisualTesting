import React, { useEffect, useState } from 'react';
import { useParams, Link, useNavigate } from 'react-router-dom';
import { getReport, getScreenshotUrl, downloadReport, deleteReport } from '../api';
import { ArrowLeft, CheckCircle, XCircle, Smartphone, Download, Trash2 } from 'lucide-react';

export default function ReportDetail() {
    const { id } = useParams();
    const navigate = useNavigate();
    const [report, setReport] = useState(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        async function load() {
            try {
                const data = await getReport(id);
                setReport(data);
            } catch (e) {
                console.error(e);
            } finally {
                setLoading(false);
            }
        }
        load();
    }, [id]);

    const handleDownload = async () => {
        try {
            const blob = await downloadReport(id);
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `report-${id}.zip`;
            document.body.appendChild(a);
            a.click();
            window.URL.revokeObjectURL(url);
            document.body.removeChild(a);
        } catch (error) {
            console.error('Download failed:', error);
            alert('Failed to download report');
        }
    };

    const handleDelete = async () => {
        if (!window.confirm('Are you sure you want to delete this report? This action cannot be undone.')) {
            return;
        }

        try {
            await deleteReport(id);
            alert('Report deleted successfully');
            navigate('/');
        } catch (error) {
            console.error('Delete failed:', error);
            alert('Failed to delete report');
        }
    };

    if (loading) return <div className="p-8 text-center">Loading...</div>;
    if (!report) return <div className="p-8 text-center text-red-500">Report not found</div>;

    return (
        <div className="max-w-6xl mx-auto p-6">
            <Link to="/" className="flex items-center gap-2 text-gray-600 mb-6 hover:text-blue-600">
                <ArrowLeft size={20} /> Back to Home
            </Link>

            <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
                <div className="bg-slate-800 text-white p-8">
                    <div className="flex justify-between items-start mb-2">
                        <h1 className="text-3xl font-bold">{report.site}</h1>
                        <div className="flex gap-2">
                            <button
                                onClick={handleDownload}
                                className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition"
                            >
                                <Download size={18} />
                                Download
                            </button>
                            <button
                                onClick={handleDelete}
                                className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition"
                            >
                                <Trash2 size={18} />
                                Delete
                            </button>
                        </div>
                    </div>
                    <p className="text-gray-300 mb-4">{report.url}</p>
                    <div className="flex gap-4">
                        <span className="bg-white/10 px-3 py-1 rounded-full text-sm">
                            {new Date(report.timestamp).toLocaleString()}
                        </span>
                        <span className="bg-white/10 px-3 py-1 rounded-full text-sm">
                            Duration: {report.duration}
                        </span>
                    </div>
                </div>

                {/* Stats Grid */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-slate-50 border-b">
                    <StatCard label="Routes Tested" value={report.results.length} />
                    <StatCard label="Total Viewports" value={report.results.reduce((acc, r) => acc + (r.viewports?.length || 0), 0)} />
                    <StatCard label="Screenshots" value={report.results.reduce((acc, r) => {
                        return acc + (r.viewports?.reduce((sum, vp) => sum + 1 + (vp.components?.length || 0), 0) || 0);
                    }, 0)} />
                    <StatCard label="Components" value={report.results.reduce((acc, r) => {
                        return acc + (r.viewports?.reduce((sum, vp) => sum + (vp.components?.length || 0), 0) || 0);
                    }, 0)} />
                </div>
            </div>

            {/* Visual Tests Section */}
            <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-2">
                <CheckCircle className="text-blue-600" /> Comparison Results
            </h2>
            <div className="flex flex-col gap-8 mb-12">
                {report.results.map((result, index) => (
                    <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-100">
                        <div className="p-4 border-b bg-gray-50">
                            <h3 className="font-bold text-gray-800 text-lg">{result.route}</h3>
                        </div>

                        <div className="p-4 space-y-8">
                            {/* Viewport Results */}
                            {result.viewports?.map((vpResult, vpIdx) => (
                                <div key={vpIdx} className="border rounded-lg p-4 bg-slate-50">
                                    <div className="flex items-center gap-3 mb-4">
                                        <span className="text-2xl">
                                            {vpResult.viewport === 'Desktop' ? '🖥️' : vpResult.viewport === 'Tablet' ? '📱' : '📱'}
                                        </span>
                                        <div>
                                            <h4 className="font-bold text-gray-800">{vpResult.viewport}</h4>
                                            <span className="text-xs text-gray-500">{vpResult.width} × {vpResult.height}</span>
                                        </div>
                                    </div>

                                    {/* Full Page Result */}
                                    {vpResult.fullPage && (
                                        <ComparisonBlock
                                            title="Full Page Scan"
                                            data={vpResult.fullPage}
                                            isMain={true}
                                        />
                                    )}

                                    {/* Component Results (Desktop only) */}
                                    {vpResult.components?.length > 0 && (
                                        <div className="border-t mt-4 pt-4">
                                            <h5 className="font-bold text-gray-700 mb-4 flex items-center gap-2">
                                                <span className="w-2 h-6 bg-blue-500 rounded-sm"></span>
                                                Component Analysis
                                            </h5>
                                            <div className="grid grid-cols-1 gap-6">
                                                {vpResult.components.map((comp, idx) => (
                                                    <ComparisonBlock key={idx} title={comp.name} data={comp} />
                                                ))}
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ))}

                            {/* Legacy format support (old reports without viewports) */}
                            {!result.viewports && result.fullPage && (
                                <>
                                    <ComparisonBlock
                                        title="Full Page Scan"
                                        data={result.fullPage}
                                        isMain={true}
                                    />
                                    {result.components?.length > 0 && (
                                        <div className="border-t pt-4">
                                            <h4 className="font-bold text-gray-700 mb-4 flex items-center gap-2">
                                                <span className="w-2 h-6 bg-blue-500 rounded-sm"></span>
                                                Component Analysis
                                            </h4>
                                            <div className="grid grid-cols-1 gap-6">
                                                {result.components.map((comp, idx) => (
                                                    <ComparisonBlock key={idx} title={comp.name} data={comp} />
                                                ))}
                                            </div>
                                        </div>
                                    )}
                                </>
                            )}
                        </div>

                        {result.error && (
                            <div className="p-4 bg-red-50 text-red-700 border-t border-red-100">
                                Error: {result.error}
                            </div>
                        )}
                    </div>
                ))}
            </div>
        </div>
    );
}

function ComparisonBlock({ title, data, isMain = false }) {
    if (!data) return null;

    // Status Badge Color
    const statusColor =
        data.status === 'pass' ? 'bg-green-100 text-green-700 border-green-200' :
            data.status === 'fail' ? 'bg-red-100 text-red-700 border-red-200' :
                data.status.includes('baseline') ? 'bg-blue-100 text-blue-700 border-blue-200' :
                    'bg-gray-100 text-gray-700';

    return (
        <div className={`rounded-xl ${isMain ? 'bg-slate-50 p-4 border border-slate-200' : 'bg-white'}`}>
            <div className="flex justify-between items-center mb-3">
                <h5 className={`font-semibold ${isMain ? 'text-xl' : 'text-lg text-gray-700'}`}>{title}</h5>
                <div className="flex items-center gap-3">
                    {data.accuracy && (
                        <div className="text-sm font-mono text-gray-600 flex flex-col items-end">
                            <span>Accuracy: <strong className="text-green-600">{data.accuracy}%</strong></span>
                            {data.totalPixels && (
                                <span className="text-xs text-gray-500">
                                    {data.matchingPixels?.toLocaleString()} / {data.totalPixels?.toLocaleString()} pixels match
                                </span>
                            )}
                            {data.diffPixels > 0 && (
                                <span className="text-xs text-red-600">
                                    {data.diffPixels?.toLocaleString()} pixels differ
                                </span>
                            )}
                        </div>
                    )}
                    <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase border ${statusColor}`}>
                        {data.status.replace('_', ' ')}
                    </span>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Imagebox label="Baseline" src={data.baseline} />
                <Imagebox label="Current" src={data.current} />
                <Imagebox label="Diff" src={data.diff} isDiff={true} status={data.status} />
            </div>
        </div>
    );
}

function Imagebox({ label, src, isDiff = false, status }) {
    if (!src && isDiff && status?.includes('baseline')) {
        return (
            <div className="border border-dashed rounded h-40 flex items-center justify-center bg-gray-50. text-gray-400 text-sm">
                Baseline Created
            </div>
        );
    }

    if (!src) return <div className="h-40 bg-gray-100 rounded"></div>;

    return (
        <div>
            <div className="text-xs text-center text-gray-500 mb-1 uppercase tracking-wider">{label}</div>
            <div className="border rounded overflow-hidden shadow-sm hover:shadow-md transition bg-white">
                <img
                    src={getScreenshotUrl(src)}
                    alt={label}
                    className="w-full h-auto cursor-pointer hover:scale-[1.02] transition-transform duration-200"
                    onClick={() => window.open(getScreenshotUrl(src), '_blank')}
                />
            </div>
        </div>
    );
}

function StatCard({ label, value }) {
    return (
        <div className="bg-white p-4 rounded-lg text-center shadow-sm">
            <div className="text-2xl font-bold text-gray-800">{value}</div>
            <div className="text-xs text-uppercase text-gray-500 font-semibold">{label}</div>
        </div>
    )
}
